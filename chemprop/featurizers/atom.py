...
from chemprop.utils.utils import EnumMapping, get_canonical_chiral_tag
...
class CanonicalMultiHotAtomFeaturizer(MultiHotAtomFeaturizer):
    """A subclass of :class:`MultiHotAtomFeaturizer` that uses canonical chiral tags.

    Example
    -------
    >>> from rdkit import Chem
    >>> from chemprop.utils import assign_neighbor_ranking
    >>> for smiles in ["C[C@H](O)N", "C[C@@H](N)O"]:
    ...     mol = Chem.MolFromSmiles(smiles)
    ...     assign_neighbor_ranking(mol)
    ...     featurizer = CanonicalMultiHotAtomFeaturizer.v2()
    ...     print("Smiles:", smiles)
